...
async function testRateLimit() {
	...
	try {
		...
		const { default: DailyAnalysisRateLimit } = await import(
			"./src/lib/dailyAnalysisRateLimit.js"
		);
		...
		let stats = await DailyAnalysisRateLimit.getUserStats(
			testUserEmail,
			testUserId
		);
		console.log("Initial stats:", stats);
		console.log(
			`Can analyze: ${stats.canAnalyze}, Count: ${stats.analysisCount}, Remaining: ${stats.remaining}\n`
		);

		// Test 2: Check if user can analyze
		console.log("🔍 Test 2: Check if user can perform analysis");
		let canAnalyze = await DailyAnalysisRateLimit.checkUserLimit(
			testUserEmail,
			testUserId
		);
		console.log("Rate limit check:", canAnalyze);
		console.log(
			`Can analyze: ${canAnalyze.canAnalyze}, Current: ${canAnalyze.currentCount}, Remaining: ${canAnalyze.remaining}\n`
		);
		...
		for (let i = 0; i < analysesToRecord.length; i++) {
			...
			console.log(
				`Recording analysis ${i + 1}: ${analysis.type} - ${analysis.topic}`
			);
			...
			console.log(
				`✅ Recorded: ${result.currentCount}/${result.limit}, Remaining: ${result.remaining}`
			);
		}
		...
		stats = await DailyAnalysisRateLimit.getUserStats(
			testUserEmail,
			testUserId
		);
		...
		while (currentCount < 8) {
			// Leave 2 remaining to test warning
		...
		const warningMessage = DailyAnalysisRateLimit.generateWarningMessage(
			stats.remaining
		);
		...
		const limitCheck = await DailyAnalysisRateLimit.checkUserLimit(
			testUserEmail,
			testUserId
		);
