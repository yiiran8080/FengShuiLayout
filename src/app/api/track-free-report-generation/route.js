...
export async function POST(request) {
    try {
        ...
        const userId = session?.user?.email || session?.user?.userId || 'anonymous';
        const email = session?.user?.email || null;

        console.log('📊 Tracking request received:', { userId, roomType, direction });
        ...
        const freeReportActivity = new FreeReportActivity({
            ...
            userInfo: {
                ...
                birthDateTime: userInfo?.birthDateTime ? new Date(userInfo.birthDateTime) : new Date()
            },
        ...
        if (!user) {
            ...
            user = new User({
                ...
                email,
                gender: userInfo?.gender || 'female',
                birthDateTime: userInfo?.birthDateTime ? new Date(userInfo.birthDateTime) : new Date(1996, 2, 12, 22),
                freeReportStats: {
                    totalGenerated: 1,
            ...
            console.log('📈 Updating user stats. New total:', user.freeReportStats.totalGenerated);
        }
...
export async function GET(request) {
    try {
        ...
        const recentActivities = await FreeReportActivity
            .find({ userId })
            .sort({ generatedAt: -1 })
            .limit(10)
            .select('roomType direction generatedAt status locale hasUploadedImage');
