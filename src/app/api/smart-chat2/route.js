...
風鈴會先給你一個簡單的命理分析，如果你覺得有幫助，還可以做更詳細的完整八字報告哦～🔮💕`;
	...
	// �👥 人際關係分析流程 - 使用 smart-chat 的通用模板
	generateRelationshipFlow(analysis, originalMessage) {
		// 🤖 優先使用已有的 AI 分析結果
		if (analysis.aiResponse) {
			console.log("✅ 使用已有的 AI 人際關係分析結果");
			const fengShuiGreeting = `哇～你想了解人際關係呀！風鈴最喜歡幫人解決問題啦！✨
...
記得要多主動關心身邊的人，真誠待人最重要！也可以穿戴粉色系的配飾，增加親和力哦！💕`;
...
風鈴會先給你一個簡單的分析，如果你覺得有幫助，還可以做更詳細的完整報告哦～💕`
		...
		// 備用模板回應
		return `哇～你想了解人際關係呀！風鈴最喜歡幫人解決問題啦！✨
				...
				[
					{
						role: "system",
						content:
							"你是專業且親切的風鈴，請根據用戶的具體問題提供相關的風水建議。重要指示：1. 必須使用繁體中文回應，不可使用簡體中文 2. 所有日期和月份都必須使用新歷（西曆/公曆），不要使用農歷。例如：1月、2月、3月等，避免使用農曆1月、農曆8月等表達方式 3. 不要在回應中包含字數統計如（72字）等標記 4. 保持風鈴親切可愛的語氣風格",
					},
					{
						role: "user",
						content: prompt,
...
💖 哈囉親愛的[五行]命小夥伴！讓風鈴為你解鎖2025年的${displayTopic}密碼～  
