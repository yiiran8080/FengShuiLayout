...
const common =
	"[說話方式] 用日常對話的方式，就像面對面聊天一樣，多用親切用詞，不要哈囉，你好；[要求] 1.語言簡潔流暢，避免術語堆砌，使用完整的句子且用口語化方式表達；用\"能量場\"\"行爲模式\"等現代詞匯替代傳統表述；2.不要超過200字 3.不要使用使用'#'或'*'等雜亂符號 4.條目之間使用\n符號分隔 5.【重要】必須嚴格使用正宗繁體中文，禁止出現任何簡體字，包括：根据→根據、关系→關係、现代→現代等";
...
const MINGLI_MAP = {
	...
	liuqin:
		prefix +
		"六親緣分。[分析維度] -夫妻宮：時柱{時柱天幹地支}十神（{十神屬性}）對婚姻穩定性的影響；- 年柱{年柱天幹地支}與父母宮位關系，推算原生家庭帶來的情感模式 -{日支藏幹}透出的潛意識需求。[輸出模板] 1. 情感優勢：用⭐️符號標記3個利於親密關系的特質 ；2. 潛在挑戰：用⚠️符號警示2個需注意的模式（如比劫爭合導致的三角關系風險）；3. 風水調節：推薦2種臥室擺設法（例：牀頭朝向{吉方}化解孤辰寡宿）",
	shiye:
		prefix +
	...
	jiankang:
		prefix +
		"健康預警及養生建議。[關鍵指標]-醫學命理映射 ：1.病竈器官：根據五行失衡{八字.五行分布}對應髒腑（例：木過旺→肝膽壓力）；2.流年觸發：標注{當前流年幹支}可能引發的健康風險（如歲運並臨加強火元素→心血管負擔）。[輸出要求] ：1.用❗️符號突出3項年度重點預防項目； 2.提供2個風水化解方案（例：在{臥室東方}放置白水晶平衡木氣）； 3.附上中醫養生建議（如土弱者可多喫黃色食物） ",
	ganqing:
		prefix +
	...
	juece:
		prefix +
		`決策時機。[關鍵指標] 1. 大運{大運幹支}與{當前流年幹支}關系 → 宏觀周期判斷。 2. 命宮{主星}四化{四化} → 決策風格（例：化祿在財帛宮 → 風險偏好較高）；3. 流月{當前流月幹支}與日柱{日柱}合衝 → 微觀時機捕捉。[輸出模板] ⏳ 年度關鍵窗口：1.- {月份}：{主星}逢{流月主星} → 適合啓動重大事項 ；2.避開{月份}：{衝克關系}易判斷失誤。🎯 決策增強技巧：1.每日{時辰}進行冥想（根據{時柱}能量高峯）；2.籤約時坐{方位}（例：財星在巽宮 → 東南方位）`,
	gexing:
		prefix +
