...
const common =
	"[说话方式] 用日常对话的方式，就像面对面聊天一样，多用亲切用词，不要哈喽，你好；[要求] 1.语言简洁流畅，避免术语堆砌，使用完整的句子且用口语化方式表达；用\"能量场\"\"行为模式\"等现代词汇替代传统表述；2.不要超过200字 3.不要使用'#'或'*'等杂乱符号 4.条目之间使用\\n符号分隔 5.【重要】必须严格使用正宗简体中文，禁止出现任何繁体字，包括：根據→根据、關係→关系、現代→现代等";
...
const MINGLI_MAP = {
	...
	liuqin:
		prefix +
		"六亲缘分。[分析维度] -夫妻宫：时柱{时柱天干地支}十神（{十神属性}）对婚姻稳定性的影响；- 年柱{年柱天干地支}与父母宫位关系，推算原生家庭带来的情感模式  -{日支藏干}透出的潜意识需求。[输出模板]  1. 情感优势：用⭐️符号标记3个利于亲密关系的特质 ；2. 潜在挑战：用⚠️符号警示2个需注意的模式（如比劫争合导致的三角关系风险）；3. 风水调节：推荐2种卧室摆设法（例：床头朝向{吉方}化解孤辰寡宿）",
	shiye:
		prefix +
	...
	jiankang:
		prefix +
		"健康预警及养生建议。[关键指标]-医学命理映射 ：1.病灶器官：根据五行失衡{八字.五行分布}对应脏腑（例：木过旺→肝胆压力）；2.流年触发：标注{当前流年干支}可能引发的健康风险（如岁运并临加强火元素→心血管负担）。[输出要求] ：1.用❗️符号突出3项年度重点预防项目； 2.提供2个风水化解方案（例：在{卧室东方}放置白水晶平衡木气）；  3.附上中医养生建议（如土弱者可多吃黄色食物）  ",
	ganqing:
		prefix +
	...
	juece:
		prefix +
		`决策时机。[关键指标] 1. 大运{大运干支}与{当前流年干支}关系 → 宏观周期判断。 2. 命宫{主星}四化{四化} → 决策风格（例：化禄在财帛宫 → 风险偏好较高）；3. 流月{当前流月干支}与日柱{日柱}合冲 → 微观时机捕捉。[输出模板] ⏳ 年度关键窗口：1.- {月份}：{主星}逢{流月主星} → 适合启动重大事项 ；2.避开{月份}：{冲克关系}易判断失误。🎯 决策增强技巧：1.每日{时辰}进行冥想（根据{时柱}能量高峰）；2.签约时坐{方位}（例：财星在巽宫 → 东南方位）`,
	gexing:
		prefix +
