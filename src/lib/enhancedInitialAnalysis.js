...
export class EnhancedInitialAnalysis {
	...
	static async generatePersonalAnalysis(
		...
		let response = `🔮 風鈴看了你的生日，發現你有很特別的${category}潛質呢！${categoryConfig.emoji}\n\n`;

		// 1. 命盤速讀 (基礎計算，不用AI)
		response += `**1. 命盤速讀**\n`;
		response += `八字：${bazi.year} ${bazi.month} ${bazi.day} ${bazi.hour}\n`;
		response += `五行屬性：${element}命\n`;
		response += `${categoryConfig.palace}：${this.getPalaceStar(element, category)}\n`;
		response += `   - 關鍵格局：\n`;
		response += `     ${this.getKeyPattern(element, year)}\n\n`;

		// 2-4. 使用AI生成個人化內容
		try {
			const aiAnalysis = await this.generatePersonalAIAnalysis(
				birthday,
				element,
				category,
				specificQuestion
			...
			if (aiAnalysis) {
				...
				// AI失敗時使用備用模板
				response += this.getFallbackPersonalAnalysis(
					element,
					category,
					specificQuestion
				);
			...
			// Add report recommendations only for fallback
			response += this.getReportRecommendations(category);
		}
